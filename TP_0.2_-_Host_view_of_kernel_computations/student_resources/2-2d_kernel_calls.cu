...
int main() {
  ...
  cudaMallocPitch(&d_buffer, &pitch, cols * sizeof(float), rows * sizeof(cols));  // FIXME
  ...
  //@@ use cudaMemcpy2D
  cudaMemcpy2D(host_buffer, sizeof(float) * cols, d_buffer, pitch, cols * sizeof(float), rows * sizeof(float), cudaMemcpyDeviceToHost);  // FIXME
  cudaCheckError();
